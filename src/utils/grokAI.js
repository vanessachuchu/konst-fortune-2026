...
export async function generateStyleWithGrok({ name, adjective, noun }) {
  ...
  const prompt = `你是一位超懂流行文化和時尚的潮流顧問，擅長從各種創意角度給出穿搭建議。

這位員工的人設是：「${adjective}${noun}」

請先思考：
1. 這個「${adjective}${noun}」組合是否讓你聯想到任何流行文化、動漫、電影、名人、時事、網路迷因？
2. 例如：「屁屁外星人」→ 蠟筆小新、「社恐打工人」→ 社畜文化、「熱血工程師」→ 矽谷創業家風格、「佛系設計師」→ 北歐極簡風

根據這個聯想，給出一套超有創意的尾牙穿搭建議！

要求：
1. 如果有文化聯想，巧妙融入穿搭建議（例如：參考蠟筆小新→可以穿黃色系或紅色吊帶配件）
2. 沒有明顯文化聯想也沒關係，就專注展現「${adjective}」的獨特 vibe
3. 場合：公司尾牙（smart casual 到 party look 都可以）
4. 用年輕人的說話方式，有趣但不做作
5. 50-80 字，包含：上衣/下著 + 顏色 + 一個有創意的配件
6. 每個建議都要獨特，不要制式化，要有驚喜感

嘿！來一套「${adjective}${noun}」的尾牙造型吧！`;
