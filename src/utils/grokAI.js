...
export async function generateStyleWithGrok({ name, adjective, noun }) {
  ...
  const prompt = `你是一位專業的穿搭顧問，擅長為尾牙活動提供實用又有個性的服裝建議。

這位員工的人設是：「${adjective}${noun}」

請給出一套適合公司尾牙的穿搭建議：

場合：公司尾牙晚宴（smart casual，可以有趣但要得體實穿）

步驟：
1. 先思考這個人設的核心特質（性格、氣質、能量感）
2. 如果聯想到流行文化梗（meme、動漫、電影等），可以巧妙融入但不要太誇張
3. 給出 1-2 個實際可執行的穿搭重點

要求：
- 建議內容：顏色系 或 單品類型 或 風格方向（擇一或擇二）
- 要實際可買到、可執行的建議
- 平衡「有趣」與「得體」，避免太 cosplay
- 40-60 字
- 語氣輕鬆友善

範例：
「佛系工程師」→ 淺灰或米白色系服裝，簡約舒適，展現從容氣質。可搭配木質手錶或素色圍巾。

「社牛行銷」→ 亮色系襯衫或特色外套，展現你的活力！配個醒目的配件讓人印象深刻。

「爆炸寶寶」→ 紅色或橘色系單品展現爆發力！可以選運動風外套或街頭感上衣，帶點可愛配件平衡一下。

「咖啡因工程師」→ 咖啡色系單品或深色休閒西裝，專業中帶輕鬆感。咖啡主題配件會很有梗！

「提示詞魔法師」→ 科技感的深藍或銀灰色系，可以加點未來感配件。簡約但有質感的設計最適合。

現在給「${adjective}${noun}」一個實用的尾牙穿搭建議：`;
